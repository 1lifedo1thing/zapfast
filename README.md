...
- **Copy text.** Select part of a message or copy across messages in
  ...
  `(DDD) XXXXX-XXXX`.
- **Keyboard shortcuts.** `Ctrl+K` searches your chats, `Ctrl+F` searches
  the open chat as in WhatsApp (Enter and Shift+Enter move between matches;
  with no chat open it searches your chats), `Ctrl+Shift+F` still works too, `Alt+↑/↓` switches chats and
